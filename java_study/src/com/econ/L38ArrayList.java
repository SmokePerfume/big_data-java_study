...
//util 패키지는 개발에 유용한 class가 다수 포함됨
//그 중에 대표적인게 자료형에 해당하는 컬렉션 프레임워크가 있다.(list, set, map)
...
public class L38ArrayList {
	public static void main(String[] args) {
		...
		System.out.println("\nc.length:"+c.length);
		//Array는 가볍고 리터럴표기법을 지원해서 선언하기 편하지만 지원해주는 함수가 전혀 없다.
		...
		System.out.println(int_list.indexOf(320));; //있으면 idnex 없으면 -1
		System.out.println(int_list.indexOf(0));
		
		System.out.println(int_list); //toString이 구현되어있다.
		System.out.println("int_list.remove(3) : "+int_list.remove(3));// 삭제한 값이 나오고 삭제한 위치만큼 한칸씩 순서를 당긴다.
		...
		list2.set(4, -10); //특정 인덱스의 요소(값)을 바꾼다.
		...
		//참조형의 Array를 ArrayList로 형변환 하는 방법 (기본형이 X)
