...
class LoginFrame extends Frame{
	...
	public LoginFrame(String name) {
		...
		//componnent.addActionListener(new Object implement ActionListener () {})
		submit.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				System.out.println(tf.getText()+" 제출 버튼이 눌러졌습니다.");	
			}
		});
		btnP.add(l);
		btnP.add(tf);
		...
		//생성하면서 바디를 구현하면 만들수 있다. 이떼 익명 클래스가 생성된다.
		//this.addWindowListener(new WindowListener(){});
		//인터페이슨 구현해야하는 것이 너무 많아서 인터페이스를 미리 구현한 추상클래스인 Adapter가 존재한다.
		this.addWindowListener(new WindowAdapter() {
			//this는 WindowAdapter기 때문에 frame를 참조하기위해 필드에 f를 등록함
