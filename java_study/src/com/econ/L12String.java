...
public class L12String {
	...
	public static void main(String[] args) {
		...
		String t3="ABCDEFGHIJKMNOPSQRS"; //=> 문자의 집합으로 바꾸세요.; ['A','B'....]
		char[] t3_arr=new char[t3.length()]; //자바의 배열은 꼭 길이를 명시해야한다.
		char[] t3_arr2= {'A','B','C'}; //자바스크립트의 Array는 고정 길이다.
		...
		System.out.println("\n");	
		System.out.println();		
		System.out.println(t3_arr);
		
		//t3_arr2[0]='A';
		//System.out.println(t3_arr2);
		
		//자바의 기본형은 byte, short, int, long, float, double, char(2byte), boolean
		//기본형 외의 모든 data는 참조형이다.
		//문자열은 왜 참조형일까?
		//1. 문자열은 인덱스를 참조하는 배열이기 때문
		//2. 자바스크립트는 문자열도 기본형인데?? 왜 자바의 문자열은 참조형이죠??
		//=> 자바스크립트의 문자열은 순수하게 문자를 참조하는 인덱스와 길이만 참조하고 스택 메모리에서 관리하기 때문에 기본형처럼 사용하는 것이지
		//=> 인덱스를 참조한다는 관점에서는 자바스크립트의 문자열도 참조형이다.
		//=> 자바의 문자열은 문자열 객체를 참조하고 있어 유용한 필드가 함게 존재하기 때문에 자바의 문자열은 절대 기본형이 될 수 없다.
		
		//문자열과 배열은 저장하는 방식(순서가 있고 길이가 있다.)이 같다.
		...
		/* 	문자열은 배열과 동일한 저장방식/
			...
			기본형의경우는 순서와길이,인덱스등만 참조하여
			리터럴화해서 사용하게되어있는형태구성/
			프론트엔드개발환경에서 효율적인 메모리사용이 장점
			 */
 	}
