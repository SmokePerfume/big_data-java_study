...
//class TeleVision extends Object{ 생략되어있다.
class TeleVision{
	LED[] oled;
	Receiver s;
	HDMI hdmi;
	...
	public TeleVision(int voltage,int chanel) {
		...
		//부모에서 정의한 이름과 같은 이름의 함수를 정의하는 것은 오버라이딩이라 하고
		//이름이 같은데 suer or this로 다르게 접근할 수 있는 상태를 다형성이라 한다.
	}
	//2. Object.String() 객체에 대한 설명서
	@Override //보모의 필드를 재정의 했다는 표시
	public String toString() {
		return "저희 tv는 "+this.p.v+"볼트에서 동작하고 LED(OLED) "+oled.length+"개 있습니다. 생산자는 "+super.toString()  ;
	}
	//부모에게 물력받은 필드는 super로 접근하고 부모에게 물려받았는데 재정의한 것은 this로 접근한다.
}
